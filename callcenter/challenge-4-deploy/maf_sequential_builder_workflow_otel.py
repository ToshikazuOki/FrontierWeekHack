...
(SequentialBuilder / Sequential Workflow パターン + Application Insights 連携版)
"""
...
# 階層構造に合わせた確実なパス計算を設定 (parents[1] = callcenter)
CALL_DATA_PATH = (
...
def init_telemetry():
    ...
    if connection_string:
        ...
        print(
            "Warning: APPLICATIONINSIGHTS_CONNECTION_STRING not set. Telemetry disabled."
        )
...
# ---------------------------------------------------------
# 5. SequentialBuilder によるワークフロー構築と実行
# ---------------------------------------------------------
...
async def run_call_center_pipeline():
    """Sequential ワークフローを用いてコールセンターのパイプライン処理を実行します（Telemetry計測付き）。"""
    ...
    with tracer.start_as_current_span("run_call_center_pipeline_workflow"):
        ...
        print(
            "=== ステップ 1: 通話データの読み込みと Sequential ワークフローの準備 ==="
        )
        if not CALL_DATA_PATH.exists():
            print(f"❌ データファイルが存在しません: {CALL_DATA_PATH}")
            sys.exit(1)
        ...
        for event in events:
            if hasattr(event, "data") and event.data:
                ...
                if (
                    hasattr(data_obj, "agent_response")
                    and data_obj.agent_response
                ):
                    agent_res = data_obj.agent_response
                    if hasattr(agent_res, "text") and agent_res.text:
                        outputs.append(agent_res.text)
                elif hasattr(data_obj, "text") and data_obj.text:
                    outputs.append(data_obj.text)

        # 最終出力（Resolution Agent の生成テキスト）を特定
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("MICROSOFT AGENT FRAMEWORK ワークフロー (Application Insights 連携) 開始")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("チャレンジ 4 (SequentialBuilder + Telemetry 移植版) 完了")
    print("=" * 60)
    print(
        "  マルチエージェント オーケストレーション (Sequential Workflow & App Insights)  ✓"
    )